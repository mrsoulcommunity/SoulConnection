...
Самый крупный выпуск на сегодня. Весь слой подключения переписан вокруг единого жизненного цикла,
слой против DPI — новый, настройки стали искомыми, а интерфейс теперь держит 60 кадров в секунду в
подключённом состоянии. Ни один канал IPC и ни один формат сообщения не изменились, поэтому
существующие серверы, подписки и правила маршрутизации переносятся нетронутыми.

### Новое

| | |
|---|---|
| **Adaptive Shield** | Слой против DPI, который *измеряет*, а не гадает. Он пробует каталог обработок на вашей сети, оставляет ту, что проходит, и запоминает ответ по хешированному отпечатку этой сети — так что сеть, которую вы уже разгадали, в следующий раз подключается сразу с нужной обработкой |
| **Поиск по настройкам** | Восемнадцать карточек на пять экранов прокрутки теперь фильтруются по любому тексту, который карточка отображает — заголовку, подсказкам, даже значениям в полях: набрав номер порта, вы найдёте карточку, где он лежит. Арабское и персидское написание одной буквы сворачиваются вместе, как и персидские с латинскими цифрами. Пароли намеренно исключены из индекса: секрет не должен становиться находимым оттого, что его набрали в строке поиска |
| **Категории уведомлений** | Главный переключатель плюс по одному на категорию — подключение/отключение, автоматическая смена сервера, обновления. Ошибки, на которые нужно реагировать, остаются вне категорий и подчиняются только главному переключателю, чтобы «сообщать о подключении» нельзя было выключить вместе с «Kill Switch не удалось применить» |
| **Настраиваемое автопереподключение** | Сколько раз повторять и с какой нарастающей паузой. Оба значения читаются на лету, поэтому изменение действует на ближайшем же обрыве, а не после перезапуска — и оба ограничиваются на входе |
| **DNS туннельного режима** | Резолверы, которые туннельный режим передаёт Windows, раньше были зашиты в код. Теперь одна настройка питает обе половины — сетевой адаптер и собственный DoH-резолвер xray, — так что они не могут разойтись. Проверяется и в поле, и повторно в главном процессе, потому что xray отвергает всю конфигурацию из-за одной плохой записи |
| **Уменьшение анимаций** | Переключатель внутри приложения рядом с системной настройкой Windows. Достаточно любого из двух; раньше учитывалась только системная |
| **Сброс настроек** | Возвращает предпочтения к значениям по умолчанию и не трогает больше ничего. Серверы, подписки, правила маршрутизации и счётчики трафика — это данные, а не предпочтения |

### Переписано

**Единый жизненный цикл подключения.** Всё, что связано с подключением — процесс xray, диспетчер
умной маршрутизации, туннельный адаптер и его маршруты, настройка прокси Windows, Kill Switch, три
цикла измерений, мониторинг состояния и переключение — раньше держалось на правилах порядка,
размазанных по `main.cjs` на 2 359 строк. Теперь это живёт в `electron/vpn/` за единым жизненным
циклом, которому каждая подсистема *следует*, а не выводит его заново. Гонки, которые это устранило,
а не переместило:

- Отключение, запрошенное во время перебора серверов, затиралось подключением, которое этот
  перебор совершал следом.
- Отключение во время обратного отсчёта автопереподключения всё равно подключало вас снова —
  отсчёт был простым таймером, и после его срабатывания никто не перепроверял ваше намерение.
- Оба цикла измерений перезапускались на каждой рассылке состояния, включая тик раз в шесть
  секунд. Перезапуск счётчика трафика сбрасывал базовую точку, и заявленная скорость несколько раз
  в минуту подскакивала до объёма всей сессии, делённого на секунду.
- «Подключиться к лучшему серверу» при уже активном подключении сообщал об *отключении* поверх
  живого туннеля, если перебор ничего не находил.
- Четыре порта диспетчера выделялись независимо, поэтому занятый порт мог достаться сразу двум
  входящим — и xray отвергал конфигурацию.
- Отмена, пришедшая посреди рукопожатия, оставляла туннель поднятым — уже объявив о подключении,
  которое вы просили прекратить.

**Перебор, за которым можно следить и который можно остановить.** Измерение группы серверов раньше
шло за спиннером, который сообщал только «что-то происходит» — полминуты этого на подписке из
пятидесяти серверов. Теперь показометром стала сама кнопка на панели: кольцо, заполняющееся по мере
измерений, живой счётчик с полосой прогресса и итог в конце. Остановка действительно останавливает —
измерения, уже находящиеся в полёте, отменяются, а не дожидаются истечения таймаутов. Брошенная так
строка возвращается к тому, что показывала раньше: её не измерили и она не падала, поэтому красный
цвет был бы выдуманным результатом.

**Шестьдесят кадров в секунду.** Телеметрия переехала из состояния React в отдельное хранилище, на
которое подписаны нижняя панель и панель туннеля напрямую, — два числа внизу больше не
перерисовывают всё приложение раз в секунду и так вечно. Результаты пинга публикуются по одному
кадру за раз вместо двух записей на измерение. Шесть полос прогресса, тень кнопки подключения и
сворачивание панели туннеля теперь анимируют transform, opacity и строку grid, а не свойства,
пересчитывающие раскладку на каждом кадре. Измерено за десять секунд подключения: 31 пропущенный
кадр из 566 стал 1 из 600, а 95-й процентиль кадра — с 33,3 мс до 16,8 мс. Перебор 400 серверов: с
10 пропущенных кадров при худшем случае 116,7 мс до нуля при худшем 16,8 мс.

**Единый язык движения.** Четыре длительности и три кривые на `:root` заменили около 65 подобранных
вручную таймингов, поэтому две панели, открывающиеся одинаково, теперь открываются с одной скоростью.

### Исправлено

- Туннель, убитый извне приложения, не запускался заново: каждое переподключение падало с «Xray
  уже запущен», пока не перезапустишь само приложение. Признак жизни теперь определяется по
  событию, и убитое извне ядро восстанавливается само.
- Уведомления об ошибках отображались в зелёном цвете успеха — на всех путях ошибки терялся флаг
  `'error'`.
- Всплывающее сообщение на 2,6 с закрывало всю строку состояния, стирая длительность, скорость,
  пинг и имя сервера ровно в момент подключения. Теперь оно висит над строкой.
- Две разные карточки настроек назывались одинаково — «پیشرفته».
- Главный экран заявлял «сервер не выбран», пока боковая панель показывала пул Soul или
  автоматический выбор — а это настоящие выборы.
- Одна шкала задержки вместо трёх: 350 мс раньше читались как янтарное «средне» в строке и как
  красное «плохо» на панели прямо над ней — в один и тот же момент.
- Латинские имена серверов обрезались с *начала* — «Tokyo — NTT Premium» показывалось как
  «…NTT Premium», теряя опознавательную половину.
- Шесть выпадающих списков в настройках имели шесть разных ширин; отключённые выглядели
  кликабельными; а собственная стрелка Chromium отличалась от всех остальных стрелок в приложении
  начертанием, толщиной и цветом. Исправлено всё три.
- Скользящая пилюля переключателя режимов была на 15 px уже надписи «پروکسی سیستم» и на 15 px шире
  «تانل کامل», из-за чего текст заметно вылезал за её пределы.
- Состояние блокировки Kill Switch не доходило до строки состояния.
- Ctrl+F внутри собственного поля поиска боковой панели открывал поиск серверов, а Ctrl+K
  открывал его позади открытого окна.
- Несколько латинских фрагментов внутри RTL-текста переставлялись алгоритмом bidi через перенос
  строки — `Host/Port/احراز هویت` отображалось как «I/Host/Port».

### Под капотом

`npm test` теперь прогоняет **98 тестов** на встроенном раннере Node — без зависимостей, без сети,
без Windows и без бинарника xray. Покрыт слой координации — именно там жили исторические ошибки:
порядок, отмена и откат. См. [Сборка из исходников](#сборка-из-исходников).
