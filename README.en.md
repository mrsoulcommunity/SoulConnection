...
A release entirely in the interface. Nothing about connecting changed — no IPC channel, no
setting, no stored file — so servers, subscriptions and routing rules carry over untouched.
What changed is that the app can now be driven without a mouse and read out loud by a screen
reader, its type comes from one scale instead of thirty hand-picked sizes, and the small
controls are big enough to actually hit.

### Fixed

| | |
|---|---|
| **Choosing a server needed a mouse** | The primary action of the whole app — picking what to connect to — sat on a `div` with an `onClick`: no tab stop, no Enter key, no announced state. In an app otherwise driven by shortcuts (Ctrl+K, Ctrl+V, Escape everywhere), it was the one thing a keyboard could not do. It is a button now, and it says which server is the chosen one |
| **Server names were unreadable** | The protocol tag sat ahead of the name in the row's flex flow, and with the ping pill and delete button alongside it, the name got about 58px of a 296px sidebar — every server truncated, including the part that identifies it. The tag moved down to the line that already carries secondary detail, and the name took the width back |
| **Icon buttons had no name** | Around forty buttons across the app were an SVG glyph and nothing else — a screen reader announced them as "button". Every one now carries a Persian label, including the three that navigate between the app's own screens |
| **Settings labels pointed at nothing** | In the settings primitives and the twenty-odd fields of the Add Config form, each label was a scrap of text sitting *near* its control rather than attached to it. Clicking a label did nothing and the inputs read out unnamed. The label now mints an id and hands it to whichever control it wraps, so no call site has to remember |
| **Errors were shown but never announced** | Validation messages appeared silently. They now carry `role="alert"`, and the Add Config form — long enough to scroll, with its error landing at the very bottom — moves focus to the message so it is read out where it is |
| **The password reveal excluded the keyboard** | The show/hide toggle on the proxy password was `tabIndex={-1}`, which excluded exactly the people who cannot fall back to "just look at the dots". It takes a real tab stop now and reports whether the password is showing |
| **Deleting a routing rule asked nothing** | One click on a 24px icon, sitting next to *edit*, with no confirmation and no undo. Every other delete in the app already asks; this one is no less permanent, and now it asks too |
| **The document had no `h1`** | Headings started at `h3`, so the outline a screen reader builds began mid-tree. The screen title is the `h1` it always looked like |

### Changed

- **One type scale.** Font sizes were picked ad hoc across 4,700 lines of CSS. They now come
  from nine variables, `--fs-1` through `--fs-9`, from the 10.5px of a unit tag to the 19px of
  the hero's numbers.
- **Honest hit targets.** Several controls were the size of their glyph and no more — the
  password toggle at 13×21, the per-rule enable switch at 32×19, the favourite star exactly
  at the 24px floor with no margin. The glyphs are unchanged; the targets around them grew.
- **Contrast.** The smallest text in the Server Finder sat at 4.09:1 against its chip, under
  the 4.5:1 small text needs.
- **Focus is visible.** Controls that gained keyboard reach gained a focus ring to match.
- **The search box is a label.** Clicking anywhere in the routing search box puts the caret in
  the field, rather than only the 18px-tall input inside its padding.
